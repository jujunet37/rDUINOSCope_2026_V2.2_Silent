...
## V2.1 major improvements : ASCOM Driver, code optimization and new search engine.
...
* All String removed, migration to char to avoid unpredictable memory issues.
* New motor moving logic for slew, with included Arduino Due hardware timer to improve CPU performance and avoid ASCOM reading conflict.
* Speed also increase by 30% (was already doubled with previous version), so it is now 2.5x times faster to slew comparing to original version. Motors spin with less vibrations.
...
* New loading menu with brand new search engine.
...
* GOTO Park Position
* Set Park feature
* Set Tracking feature
* Set Rates feature
* Pier Side position
* Synchronisation
* RA/DEC reading coordinates
* ALT/AZ coordinated (calculated by the driver)
* Import GPS coordinates (LAT, LONG, ALT)
* Manual move with invert axis
* Abort move
* Autoguiding (pulse guiding) No more need an external device.
...
* Several stars names corrected in alignment list.
* RA/DEC coordinates : A rounding bug in coordinates made a 1m or 1' jump.
* Files on SD card may be corrupted after going into options menu (was caused by a String value)
* Meridian flip didn't occur when goto back on east and pass it a second time.
* Brightness setting woundn't work.
* BT manual move amount corrected, adaptative amount depending on DEC value
* BT slew froze the APP.
* BT slew wouldn't show telescope position during a Slew
* Display bug on OBSERVING / TRACKING line
...
* Motors start to move when shutdown the PC with tracking ON (even with driver disconnected). Set tracking OFF before shut down the PC.
...
* Windows settings : Must change decimal character to "." in region settings
...
* marekburiak-ILI9341\_due-13edbd2
...
* Arduino SAM Boards (32-bits ARM Cortex-M3) (v 1.6.12)
...
# Update 16/07/2024 : rDUINOScope\_2024\_Big\_Update V2.0



### Corrected bugs and many new features and catalogs :
...
* It uses a modified version of marekburiak-ILI9341\_due library that has been modified specially for this project.
