...
void planet_pos(int pno)
{
  ...
  if (pno != 3 && pno != 10)
  {
    ...
    dec_x[pno] = atan(Zq[pno] / sqrt(pow(Xq[pno], 2.0) + pow(Yq[pno], 2.0)));

    OBJ_RA  = FNdegmin((ra[pno] * degs) / 15);
    OBJ_DEC = FNdegmin(dec_x[pno] * degs);
  ...
  else if (pno == 10)
  {
    ...
    double D  = 297.8501921  + 445267.1114034 * T  - 0.0018819 * T2 + T3 / 545868 - T4 / 113065000; // JG modif updated values
    while (D > 360)  D -= 360;
    while (D < 0) D += 360;
    ...
    double Mm  = 134.9633964 + 477198.8675055 * T  + 0.0087414 * T2 + T3 / 69699  - T4 / 14712000; //Moon's mean anomaly // JG modif updated values
    while (Mm > 360) Mm -= 360;
    while (Mm < 0) Mm += 360;

    double F   = 93.2720950 + 483202.0175233 * T   - 0.0036539 * T2 - T3 / 3526000 + T4 / 863310000; // JG modif updated values
    while (F > 360) F -= 360;
    while (F < 0) F += 360;
    ...
    if (alpha < 0) {alpha = 360+alpha;} // Add JG corrected bug sometimes RA went false when alpha is negative
    ...
    OBJECT_RA_H = abs(floor(alpha)); //Add JG (abs) corrected bug sometimes RA went negative
